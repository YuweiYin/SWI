...
def score(
    ...
    Args:
        ...
        - :param: `cache_dir` (Optional[str]): The directory to cache HF models and tokenizers.
    ...
    tokenizer = get_tokenizer(model_type, use_fast_tokenizer, cache_dir=cache_dir)
    model = get_model(model_type, num_layers, all_layers, cache_dir=cache_dir)
    if device is None:
...
def plot_example(
    ...
    tokenizer = get_tokenizer(model_type, use_fast_tokenizer, cache_dir=cache_dir)
    model = get_model(model_type, num_layers, cache_dir=cache_dir)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model.to(device)
