...
def get_model(model_type, num_layers, all_layers=None, cache_dir=None):
    if model_type.startswith("scibert"):
        model = AutoModel.from_pretrained(cache_scibert(model_type), cache_dir=cache_dir)
    elif "t5" in model_type:
        from transformers import T5EncoderModel

        model = T5EncoderModel.from_pretrained(model_type, cache_dir=cache_dir)
    else:
...
def get_tokenizer(model_type, use_fast=False, cache_dir=None):
    if model_type.startswith("scibert"):
        model_type = cache_scibert(model_type)

    if version.parse(trans_version) >= version.parse("4.0.0"):
        tokenizer = AutoTokenizer.from_pretrained(model_type, use_fast=use_fast, cache_dir=cache_dir)
    else:
