...
from evaluate_metrics import comet_score as comet
import datasets
import torch
...
class COMET(evaluate.Metric):
    ...
    def _download_and_prepare(self, dl_manager):
        download_dir = os.path.join(dl_manager.download_config.cache_dir, "downloads/comet")
        ...
        if self.config_name == "default":
            if version.parse(comet.__version__) >= version.parse("2.0.0"):
                self.scorer = comet.load_from_checkpoint(
                    comet.download_model("Unbabel/wmt22-comet-da", saving_directory=download_dir))
            else:
                self.scorer = comet.load_from_checkpoint(
                    comet.download_model("wmt20-comet-da", saving_directory=download_dir))
        else:
            self.scorer = comet.load_from_checkpoint(
                comet.download_model(self.config_name, saving_directory=download_dir))
    ...
    def _compute(self, sources, predictions, references, gpus=None, progress_bar=False):
        ...
        if version.parse(comet.__version__) >= version.parse("2.0.0"):
            output = self.scorer.predict(data, gpus=gpus, progress_bar=progress_bar, num_workers=1)
